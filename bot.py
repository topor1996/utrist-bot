...
def main():
    ...
    # Обработчик ответа администратора на вопрос (ПЕРЕД unified_message_handler!)
    # Проверяет, идет ли процесс ответа на вопрос, и если да, обрабатывает ответ
    # Если не идет процесс ответа, просто возвращается, позволяя unified_message_handler обработать сообщение
    application.add_handler(MessageHandler(
