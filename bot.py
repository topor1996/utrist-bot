...
def main():
    ...
    # Обработчик упрощенной записи (ДОЛЖЕН БЫТЬ ПЕРЕД обработчиком услуг!)
    # Проверяет, есть ли активный процесс записи в user_data
    # Если есть - обрабатывает, если нет - пропускает
    # Важно: этот обработчик должен быть первым, чтобы он обрабатывал сообщения во время процесса записи
    application.add_handler(MessageHandler(
        filters.TEXT & 
        ~filters.COMMAND & 
    ...
    # Обработчики конкретных услуг (должны быть ПОСЛЕ обработчика записи!)
